...
const InterestModal = ({ isOpen, onClose, onSave, userId }: { isOpen: boolean, onClose: () => void, onSave: () => void, userId: string | null }) => {
        ...
        <AnimatePresence>
            {isOpen && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ duration: 0.3, ease: "easeOut" }} className="bg-gray-900 border border-white/20 rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="p-6 overflow-y-auto">
                            <p className="text-gray-400 mb-6">Choose topics you're passionate about for personalized event suggestions.</p>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                {availableInterests.map(interest => {
                                    const isSelected = selectedInterests.includes(interest.name);
                                    return (
                                        <motion.div key={interest.key} onClick={() => handleInterestToggle(interest.name)} className={`p-4 rounded-lg cursor-pointer border-2 transition-all duration-200 ${isSelected ? 'border-purple-500 bg-purple-500/20' : 'border-gray-700 bg-gray-800 hover:border-gray-600'}`} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                        ...
                        <footer className="p-6 mt-auto border-t border-white/10">
                            ...
                            <button onClick={handleSave} disabled={isSaving || selectedInterests.length === 0} className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 py-3 rounded-full shadow-lg hover:scale-105 transform transition-all duration-300 font-semibold disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100">
...
export default function EventSuggestionsPage() {
    ...
    const [userInterests, setUserInterests] = useState<EventData[]>([]);
        ...
        <>
            ...
            <div className="min-h-screen bg-gradient-to-br from-gray-900 via-slate-900 to-black text-white p-4 sm:p-6 lg:p-8 pt-20 sm:pt-24 mt-20">
                <div className="max-w-7xl mx-auto">
                    <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-col sm:flex-row justify-between items-center text-center sm:text-left mb-12 gap-4">
                        <div>
                        ...
                        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 bg-white/10 px-5 py-2.5 rounded-full font-semibold hover:bg-white/20 transition-colors flex-shrink-0">
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-10 text-red-400 flex flex-col items-center gap-2">
                            <AlertCircle />
                            <p>{error}</p>
                            <button onClick={fetchInitialData} className="mt-4 px-4 py-2 bg-white/10 rounded-lg hover:bg-white/20 transition">Try Again</button>
                        </div>
