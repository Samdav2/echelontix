...
const EventCard = ({ title, location, date, time, image, price, category, status }: EventCardProps) => {
  ...
  return (
    <div className={`bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-transform duration-300 cursor-pointer group flex flex-col ${status === 'past' ? 'opacity-60' : ''}`}> 
      ...
      <div className="p-4 flex flex-col flex-grow">
        <span className="text-xs text-purple-400 bg-purple-900/30 px-2 py-1 rounded-full self-start mb-2">{category}</span>
        ...
        <div className="flex justify-between items-center mt-auto pt-3 border-t border-gray-700/50">
          ...
          <button className={`px-4 py-2 text-sm font-semibold rounded transition-all duration-300 ${getButtonStyle()}`} disabled={status === 'past'}>
            {getButtonText()}
          </button>
        </div>
      </div>
