...
// Define the structure of the event data fetched from the API
...
const EventForm: React.FC = () => {
  ...
  useEffect(() => {
    // Get the eventId from the URL, e.g., /registration?eventId=54
    ...
    const fetchEventDetails = async () => {
      ...
      try {
        ...
        if (response.data && response.data.event && response.data.event.length > 0) {
  ...
  const handleFormSubmit = (e: React.FormEvent<HTMLFormElement>) => {
      e.preventDefault();
      // Here you would handle the form submission, e.g., send to another API
      console.log("Form submitted with data:", formData);
      alert("Registration submitted! (See console for data)");
  }

  // A helper to format the date string
  ...
  // A helper to generate ticket options
          ...
          <form onSubmit={handleFormSubmit} className="space-y-4">
            <div className="flex gap-3">
              <input
                ...
                className="w-1/2 px-4 py-2 bg-gray-800 border border-gray-700 rounded text-sm focus:outline-none focus:ring-1 focus:ring-yellow-400"
                required
              />
              <input
                type="text"
            ...
            <input
              ...
              className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded text-sm focus:outline-none focus:ring-1 focus:ring-yellow-400"
              required
            />
            ...
            <select
              ...
              onChange={handleInputChange}
              className="w-full px-4 py-2 bg-gray-800 border border-gray-700 rounded text-sm text-gray-300 focus:outline-none focus:ring-1 focus:ring-yellow-400"
              required
            >
