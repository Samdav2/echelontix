...
const Header: React.FC<{ isSignedIn: boolean }> = ({ isSignedIn }) => {
  return (
    <header className="bg-gray-800 p-4 text-white flex justify-between items-center">
      <h1 className="text-xl font-bold">ECHELONTIX</h1>
      {isSignedIn && <div>User Profile</div>}
    </header>
  );
};
...
const EventCard = ({
  ...
  const placeholderImg = "https://placehold.co/240x320/1a1a1a/ffffff?text=Event";
  ...
  return (
    <div className={`bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-lg overflow-hidden hover:scale-105 transition-transform duration-300 cursor-pointer relative w-[240px] h-[380px] flex flex-col ${status === 'past' ? 'opacity-70' : ''}`}>
      ...
      <div className="p-4 flex flex-col flex-grow">
        ...
        <h3 className="text-md font-semibold text-white mb-2 flex-grow" title={title}>{title}</h3>
        ...
        <div className="flex justify-between items-center mt-auto pt-2 border-t border-gray-700">
          ...
          <button className={`px-3 py-1.5 text-xs rounded transition-all duration-300 ${getButtonStyle()}`} disabled={status === 'past'}>
            {getButtonText()}
          </button>
        </div>
      </div>
...
const predefinedCategories = ["Concert", "Party", "Sports", "Tech", "Food & Drink"];
...
const LandingPage = () => {
  ...
  // This will be calculated only on the client-side
  const [isSpotlightPast, setIsSpotlightPast] = useState(false);

  useEffect(() => {
    // This effect runs only once on the client
    setIsClient(true);
    const spotlightEventDate = new Date('2025-01-30');
    setIsSpotlightPast(new Date() > spotlightEventDate);
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        predefinedCategories.forEach(cat => { processedEvents[cat] = []; });
    ...
    <main className="w-full bg-black text-white font-sans">
      ...
      <section className="relative w-full flex flex-col lg:flex-row items-center justify-between px-6 lg:px-24 pt-20 pb-4 bg-black">
        ...
        <div className="relative mt-10 lg:mt-0 w-80 h-80">
          <Image src="/assets/booth.png" alt="Tickets Booth" layout="fill" objectFit="contain" />
        </div>
      </section>
      ...
      <section className="bg-black text-white pt-8 pb-16 px-4">
        <div className="flex justify-center gap-4 flex-wrap mb-10">
          {categories.map((cat) => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-6 py-2 text-sm font-semibold rounded-full transition ${activeCategory === cat ? "bg-yellow-400 text-black" : "bg-gray-800 hover:bg-gray-700"}`}>
              {cat}
            </button>
          ))}
        </div>
      ...
      <section className="relative w-full h-[550px] bg-black text-white overflow-hidden">
        <Image src="/assets/stage.svg" alt="Event Background" layout="fill" objectFit="cover" className="opacity-70" priority />
        <div className="relative z-10 flex flex-col items-center justify-center h-full text-center px-4">
          <h3 className="text-lg md:text-xl font-semibold uppercase">The Lewistage Theatre Event</h3>
          <h1 className="text-3xl md:text-5xl font-bold mt-2">May 30th – June 5th</h1>
          <div className="mt-8 bg-white text-black rounded-md p-4 md:p-6 max-w-4xl shadow-xl flex flex-col md:flex-row items-center justify-between gap-4 border-l-[8px] border-yellow-400">
            <div className="flex flex-col gap-2 w-full md:w-2/3">
              ...
              <h2 className="text-xl md:text-2xl font-bold">LASU Food Festival</h2>
              ...
              <div className="flex items-center gap-4 mt-4">
                {!isSpotlightPast ? (
                  <button onClick={() => handleEventClick(0, 'upcoming')} className="bg-yellow-400 hover:bg-yellow-500 transition px-5 py-2 text-sm font-semibold rounded">Get Tickets</button>
                ) : (
            ...
            <div className="w-full md:w-1/3 relative h-48">
              <Image src="/assets/lasu-food.svg" alt="LASU Food Festival" layout="fill" objectFit="contain" className="rounded" />
            </div>
          </div>
          <div className="absolute left-4 top-1/2 transform -translate-y-1/2 z-20 bg-yellow-400 w-10 h-10 flex items-center justify-center rounded-full cursor-pointer"><span className="text-black font-bold text-xl">&larr;</span></div>
          <div className="absolute right-4 top-1/2 transform -translate-y-1/2 z-20 bg-yellow-400 w-10 h-10 flex items-center justify-center rounded-full cursor-pointer"><span className="text-black font-bold text-xl">&rarr;</span></div>
        </div>
      </section>
    </main>
  );
